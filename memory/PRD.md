...
## Changelog
- 2026-06 (форк): Реализованы Задачи 12, 14, 15 (последние из брифа на 16 пунктов). Все 16 задач ✅.
  - Задача 12: справочник «Быстрые комментарии» (quick_comments, контексты dish/order/cancel, CRUD в /admin/settings, чипы+свой текст в POS, печать комментария на тикете); внесение/изъятие налички (cash_movements, POST/GET /api/shifts/cash-movement, кнопка «Касса» в POS для admin, суммы cash_in/cash_out/expected_cash в закрытии смены); обязательная причина при отмене заказа (DELETE /api/orders/{id} с reason при отправленном заказе, лог в order_corrections, модалка с чипами в POS).
  - Задача 14: настраиваемые способы оплаты (payment_methods, CRUD в /admin/settings, is_debt/active, динамические кнопки в POS, валидация кода при оплате, защита от дублей кода); счёт клиента «в долг» (debt_balance, оплата в долг требует клиента, POST /api/clients/{id}/pay-debt для manager+admin, отчёт /api/reports/debts + вкладка «Долги», колонка «Долг» и модалка погашения в /admin/clients); кэшбэк не начисляется на оплату в долг; totals_by_method в закрытии смены.
  - Задача 15: курсы подачи (course_number на категории → наследуется продуктом, переопределение на продукте; в /admin/menu поля «Курс»; группировка корзины POS по «— Подача N —»; печать заголовков «-- Подача N --» и комментариев на кухонных тикетах; превью тикетов в POS показывает курсы и комментарии).
  - Мультитенантность: справочники (payment_methods, quick_comments) сидируются при создании нового заведения (POST /api/restaurants).
  - Тестирование: iteration_14 — 30/30 бэкенд-тестов, фронтенд-потоки пройдены; критичный баг роли pay-debt (manager 403) исправлен и проверен end-to-end.

## Changelog (доп. улучшения после 16 задач)
- 2026-06: 4 пользовательских улучшения (бэкенд+фронтенд, протестировано iteration_15):
  - **Z-отчёт по смене**: close_shift возвращает сериализованную смену + движения налички + totals_by_method; в POS модалка Z-отчёта авто-появляется после «Закрыть смену» (выручка по способам оплаты, продажи/внесения/изъятия наличных, ОЖИДАЕМО В КАССЕ, кнопка «Печать»). Баг размещения модалки (за early-return при shift===null) исправлен — модалка рендерится в обеих ветках, zReport сбрасывается при открытии смены.
  - **Курсовая подача**: send_order группирует по (workshop_id, course_number) → отдельный тикет/задание печати на каждый курс; ответ tickets теперь МАССИВ {workshop, course_number, items}; превью тикетов в POS показывает блок на каждый курс. Тесты test_printing.py/test_pos_flow.py обновлены под новую структуру.
  - **История долга клиента**: GET /clients/{id}/debt-transactions; в /admin/clients иконка «История долга» открывает модалку со списком начислений/погашений (дата, сумма ±, остаток, локализованный способ оплаты).
  - **Экспорт отчётов CSV**: /app/frontend/src/lib/csv.js (BOM+';' для Excel-кириллицы); кнопки «Экспорт CSV» на вкладках Отчётов (Продажи по позициям/по клиентам, Маржинальность, Акции, Возвраты, Долги, Удаления), генерация полностью на клиенте.
  - Известное: 2 старых теста в test_pos_flow.py падают из-за всегда активной демо-акции «Счастливые часы −15%» (предсуществующее, не связано с этими задачами).


## Changelog (ещё 3 улучшения)
- 2026-06: Гибкие подачи, лимит долга, расписание акций (протестировано iteration_16, 92%→фиксы применены):
  - **Гибкие подачи (fire по курсам)**: send_order принимает `?course=N` — отправляет на кухню только позиции этого курса; в POS на каждой группе «— Подача N —» кнопка «Отправить подачу» (fire-course-{N}), появляется только при наличии неотправленных позиций курса. Глобальная «Отправить на кухню» сохранена. Закрытие модалки тикетов больше НЕ выходит из заказа (остальные курсы остаются редактируемыми).
  - **Лимит долга клиента**: ClientReq.credit_limit (0 = без лимита); поле «Лимит долга» в карточке клиента. В POS при оплате «в долг», если прогнозируемый долг (текущий + чек) > лимита, показывается модалка-предупреждение (debt-warn-modal) с «Отмена»/«Всё равно провести» (warn+override, бэкенд не блокирует). Исправлен HIGH-баг: правка клиента больше не обнуляет лимит.
  - **Расписание акций**: было уже реализовано (backend promo_is_active + UI полей время/дни). Демо-акции «Счастливые часы −15%» задано окно 15:00–18:00 (UTC), все дни. ВНИМАНИЕ: окна времени сравниваются по UTC — при необходимости локального времени нужен tz-параметр заведения.
  - Тех.долг: F811 (route param `date` затенял datetime.date) исправлен через Query(alias="date"). Известное: Pos.jsx ~1070 строк — кандидат на разбиение; демо-роутинг «Греческий салат → Бар» — косметика сид-данных.
