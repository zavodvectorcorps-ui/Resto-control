  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {workshops.map((w) => (
          <div key={w.id} className="bg-[#121212] border border-[#27272A] rounded-xl p-6 flex items-center justify-between" data-testid={`workshop-${w.id}`}>
            ...
            <button onClick={() => del(w.id)} className="text-[#A1A1AA] hover:text-[#FF3B30]" data-testid={`del-workshop-${w.id}`}><Trash2 size={16} /></button>
          </div>
        ))}
      </div>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title="Новый цех">
        <div className="space-y-4">
          ...
          <div>
            <label className="text-xs uppercase tracking-[0.15em] text-[#A1A1AA]">Цвет</label>
